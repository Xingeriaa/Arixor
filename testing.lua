...
game:GetService("RunService").Stepped:Connect(function()
    if _G.itemcollectingCD and _G.itemcollectingtoggler then 
        ...
        local ComparedNumber = 9999999999999999999999999999;
        local TheChosen = nil;
            for i, v in pairs(game:GetService("Workspace").Item:GetChildren()) do
                if v:IsA("Part") then 
                    if (v.Position.Y > -50 and v:FindFirstChild("ProximityPrompt") and v.Name ~= "Chest") or (v:IsA("Part") and v.Position.Y > -50 and v:FindFirstChild("ProximityPrompt") and v.Name == "Chest" and (char:FindFirstChild("Chest Key") or player.Backpack:FindFirstChild("Chest Key"))) then 
                        ItemDistance = math.round(hrp.Position-v.Position).magnitude
                        if (ComparedNumber<ItemDistance) then
                            ComparedNumber = ItemDistance;
                            TheChosen = v;
                        end
                    end
                end
            end
            if TheChosen.Position.Y > -50 and TheChosen:FindFirstChild("ProximityPrompt") then 
                ImproTheChosenedTeleport(TheChosen);
                wait()
                pcall(function() 
                    fireproximityprompt(TheChosen:FindFirstChild("ProximityPrompt"), (hrp.Position - TheChosen.Position).magnitude);
                end)
                wait(1)
            end
